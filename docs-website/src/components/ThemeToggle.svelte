...
/>
<label
	class="color-mode-toggle-label"
	for="color-mode-toggle"
	data-active={lightMode}
>
	<span>
		{#if !lightMode}
...
<style>
	...
	.color-mode-toggle-label {
		...
		border-radius: 100vh;
		outline: 1px solid var(--clr-theme);
		cursor: pointer;
    position: relative;
	}

	.color-mode-toggle-label span {
		display: flex;
	...
	.color-mode-toggle-label[data-active='true'] span {
		transform: translateY(-50%) translateX(1rem);
	}

	.color-mode-toggle-label svg {
		width: 0.75rem;
		fill: #fff;
	}
</style>